...
using CVector3 = glm::tvec3<float>;
using CVector2 = glm::tvec2<float>;
using tVector3i = glm::tvec3<int32_t>;
...
struct tBSPVisData {
    ...
    uint8_t *pBitsets;                // The array of bytes that holds the cluster bitsets
};
...
private:
    ...
    void ChangeGamma(uint8_t *pImage, int size, float factor);

    // This creates a texture map from the lightmap image bits
    void CreateLightmapTexture(uint32_t &texture, uint8_t *pImageBits, int width, int height);
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
    ...
    int m_numOfVerts = 0;            // The number of verts in the model
    int m_numOfFaces = 0;            // The number of faces in the model
    int m_numOfIndices = 0;            // The number of indices for the model
    int m_numOfTextures = 0;        // The number of texture maps
    int m_numOfLightmaps = 0;        // The number of light maps
    int m_numOfNodes = 0;            // The number of nodes in the BSP
    int m_numOfLeafs = 0;            // The number of leafs
    int m_numOfLeafFaces = 0;        // The number of faces
    int m_numOfPlanes = 0;            // The number of planes in the BSP
    int m_numOfBrushes = 0;            // The number of brushes in our world
    int m_numOfBrushSides = 0;        // The number of brush sides in our world
    int m_numOfLeafBrushes = 0;        // The number of leaf brushes

    int m_traceType = 0;            // This stores if we are checking a ray, sphere or a box
    float m_traceRatio = 0;            // This stores the ratio from our start pos to the intersection pt.
    float m_traceRadius = 0;        // This stores the sphere's radius for a collision offset

    bool m_bCollided = false;            // This tells if we just collided or not


/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *

    bool m_bGrounded = false;            // This stores whether or not we are on the ground or falling
    bool m_bTryStep = false;            // This tells us whether or not we should try to step over something

/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *

    CVector3 m_vTraceMins = {0, 0, 0};        // This stores the minimum values of the AABB (bottom corner)
    CVector3 m_vTraceMaxs = {0, 0, 0};        // This stores the maximum values of the AABB (top corner)
    CVector3 m_vExtents = {0, 0, 0};        // This stores the largest length of the box
    CVector3 m_vCollisionNormal = {0, 0, 0};    // This stores the normal of the plane we collided with

    int              *m_pIndices = nullptr;        // The object's indices for rendering
    tBSPVertex    *m_pVerts = nullptr;        // The object's vertices
    tBSPFace      *m_pFaces = nullptr;        // The faces information of the object
    tBSPNode      *m_pNodes = nullptr;        // The nodes in the BSP
    tBSPLeaf      *m_pLeafs = nullptr;        // The leafs in the BSP
    tBSPPlane     *m_pPlanes = nullptr;        // The planes in the BSP
    int           *m_pLeafFaces = nullptr;    // The leaf faces in the BSP
    tBSPVisData   m_clusters = {};        // The cluster info for frustum culling and portals (PVS)
    tBSPTexture   *m_pTextures = nullptr;        // This stores our texture info for each brush
    tBSPBrush      *m_pBrushes = nullptr;        // This is our brushes
    tBSPBrushSide *m_pBrushSides = nullptr;    // This holds the brush sides
    int              *m_pLeafBrushes = nullptr;  // The indices into the brush array

    uint32_t m_textures[MAX_TEXTURES];    // The texture array for the world
    uint32_t m_lightmaps[MAX_TEXTURES];    // The lightmap texture array
